...
void MenuSystem::displayMainMenu()
{
    ...
    for (int i = 0; i < option1.length(); ++i) grid[startRow + 3][option1Col + i] = option1[i];
    for (int i = 0; i < option2.length(); ++i) grid[startRow + 4][option2Col + i] = option2[i];
    for (int i = 0; i < option3.length(); ++i) grid[startRow + 5][option3Col + i] = option3[i];
    for (int i = 0; i < prompt.length(); ++i) grid[startRow + 7][promptCol + i] = prompt[i];
...
void MenuSystem::displayDeathScreen()
{
    ...
    for (int i = 0; i < title.length(); ++i) grid[startRow][titleCol + i] = title[i];
    for (int i = 0; i < msg1.length(); ++i) grid[startRow + 2][msg1Col + i] = msg1[i];
    for (int i = 0; i < msg2.length(); ++i) grid[startRow + 3][msg2Col + i] = msg2[i];
    for (int i = 0; i < msg3.length(); ++i) grid[startRow + 5][msg3Col + i] = msg3[i];
    for (int i = 0; i < option1.length(); ++i) grid[startRow + 7][option1Col + i] = option1[i];
    for (int i = 0; i < option2.length(); ++i) grid[startRow + 8][option2Col + i] = option2[i];
    for (int i = 0; i < prompt.length(); ++i) grid[startRow + 10][promptCol + i] = prompt[i];
...
void MenuSystem::displayExitScreen()
{
    ...
    for (int i = 0; i < farewell1.length(); ++i) grid[startRow][col1 + i] = farewell1[i];
    for (int i = 0; i < farewell2.length(); ++i) grid[startRow + 2][col2 + i] = farewell2[i];
    for (int i = 0; i < farewell3.length(); ++i) grid[startRow + 3][col3 + i] = farewell3[i];
    for (int i = 0; i < farewell4.length(); ++i) grid[startRow + 5][col4 + i] = farewell4[i];
